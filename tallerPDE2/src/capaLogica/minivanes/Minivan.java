...
public class Minivan implements Serializable {
	...
	public LinkedList<VOPaseo> listarPaseosAsignados() {
		...
		while (iter.hasNext()) {
			Paseo pas = iter.next();
			VOPaseo vpas = new VOPaseo(pas.getCodigo(), pas.getDestino(), pas.getHoraPartida(), pas.getHoraLlegada(),
					pas.getPrecioBase(), pas.cantMaxBoletos(this), pas.cantBolDisp(this));
			VOPaseos.add(vpas);
		}
		return VOPaseos;
	}
