...
public class Boletos implements Serializable {
	...
	public List<VOBoleto> printBoletos() {
		...
		for (int i = 0; i < tope; i++) {
			VOBoleto boletoVO = new VOBoleto(boletos[i].getNumBoleto(), boletos[i].getNombre(), boletos[i].getEdad(),
	...
	// Calcula el total de ventas de boletos, considerando los descuentos para
	// boletos especiales
